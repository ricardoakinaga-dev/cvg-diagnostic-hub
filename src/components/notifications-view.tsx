...
export function NotificationsView() {
  ...
  const load = useCallback(async () => {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível carregar as notificações."); }
  }, [filter]);
  ...
  async function acknowledge(id: string) { try { await apiFetch(`/notifications/${id}/acknowledge`, { method: "POST", body: "{}" }); void load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível confirmar a notificação."); } }
