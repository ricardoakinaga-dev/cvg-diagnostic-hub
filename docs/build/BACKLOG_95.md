...
**Fechamento local W5/W6 (20/08/2026):** os itens técnicos foram verificados novamente no artefato servido. A evidência consolidada é `npm run test:coverage` (109 testes; 95,35% statements; 81,05% branches), typecheck/lint/build, Playwright 24/24 pela URL LAN, incluindo a regressão visual da tela de solicitação, acessibilidade 6/6, OpenAPI 47 paths, docs 56 arquivos, PostgreSQL/restore (`1|26|13`), perf smoke em quatro rotas no `next start` (400 requests, 0 erros, p95 máximo 434,69 ms contra alvo de 500 ms) e secret/audit scans limpos. `CONDITIONAL` e `BLOCKED EXTERNAL` permanecem gates de ambiente, política ou aceite; não representam aprovação hospitalar.
