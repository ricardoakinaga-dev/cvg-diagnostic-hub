...
| --- | --- | --- |
| implemented locally | request context/multi-item/protocol/duplicate/cancel; Lab sample/recollection; RX/US workflow; result draft/release/version/view/review/amend/void; patient/encounter context; workflow actions; audit; session/CSRF/RBAC/IDOR; local/S3-compatible attachments; queue/search with typed filters/keyset cursors/timeline/dashboard indicators; leased outbox/SSE; bounded metrics/readiness; versioned catalog administration; ADMIN-only user/role administration with re-authentication; role-aware technical ADMIN landing | `src/server` and `src/components` tests, API tests, 110 Vitest tests, 95.35% statements/81.05% branches, 27 Playwright E2E runs via LAN including request-detail layout and the technical ADMIN landing, explicit 6/6 accessibility suite, PostgreSQL/restore smoke (`1|26|13`), four-route production perf smoke (400 requests, 0 errors, max p95 434.69 ms), OpenAPI (47 paths) and security validation. |
