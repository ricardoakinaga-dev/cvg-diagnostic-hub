...
| --- | --- | --- |
| implemented locally | request context/multi-item/protocol/duplicate/cancel; Lab sample/recollection; RX/US workflow; result draft/release/version/view/review/amend/void; audit; session/CSRF/RBAC/IDOR; local/S3-compatible attachments; queue/search/timeline/dashboard; leased outbox/SSE; bounded metrics/readiness | `src/server` tests, API tests, 62 Vitest tests, 15 Playwright E2E runs, PostgreSQL/restore smoke, perf smoke, OpenAPI and security validation. |
